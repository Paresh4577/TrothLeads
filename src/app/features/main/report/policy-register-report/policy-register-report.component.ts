...
import { IGroupHeadDto } from '@models/dtos/transaction-master';
import { ICustomerDto } from '@models/dtos/core/CustomerDto';

@Component({
  selector: 'gnx-policy-register-report',
  templateUrl: './policy-register-report.component.html',
  styleUrls: ['./policy-register-report.component.scss'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class PolicyRegisterReportComponent {
  //#region decorator
  //#endregion decorator
  ...
  public groupHeadName$: Observable<IGroupHeadDto[]>
  ...
  private _registerReportApi = API_ENDPOINTS.Report.StandardUserRegisterReport;
 ...
 */
  ...
  public generateReport(): void {
    ...
    if (!this._authService._userProfile.value?.AuthKeys?.includes("StandardUserRegisterReport-export")) {
    ...
    //Branch
    if (this.policyRegisterForm.get("BranchId").value != "" && this.policyRegisterForm.get("BranchId").value != null && this.policyRegisterForm.get("BranchId").value != 0) {
      rules.push({ Field: "Transaction.Branch.Id", Operator: "eq", Value: this.policyRegisterForm.get("BranchId").value });
    }

    //Sale Person Type
    if (this.policyRegisterForm.get("SalesPersonType").value != "" && this.policyRegisterForm.get("SalesPersonType").value != null) {
      rules.push({ Field: "Transaction.SalesPersonType", Operator: "eq", Value: this.policyRegisterForm.get("SalesPersonType").value });
    }

    //Sales Person Name
    if (this.policyRegisterForm.get("SalesPersonId").value != "" && this.policyRegisterForm.get("SalesPersonId").value != null && this.policyRegisterForm.get("SalesPersonId").value != 0) {
      rules.push({ Field: "Transaction.SalesPersonId", Operator: "eq", Value: this.policyRegisterForm.get("SalesPersonId").value });
    }

    //Team Reference
    if (this.policyRegisterForm.get("TeamReferenceId").value != "" && this.policyRegisterForm.get("TeamReferenceId").value != null && this.policyRegisterForm.get("TeamReferenceId").value != 0) {
      rules.push({ Field: "Transaction.TeamReferenceUserId", Operator: "eq", Value: this.policyRegisterForm.get("TeamReferenceId").value });
    }

    //Group Head
    if (this.policyRegisterForm.get("GroupHeadId").value != "" && this.policyRegisterForm.get("GroupHeadId").value != null && this.policyRegisterForm.get("GroupHeadId").value != 0) {
      rules.push({ Field: "Transaction.GroupHeadId", Operator: "eq", Value: this.policyRegisterForm.get("GroupHeadId").value });
    }
    ...
    //BDO
    if (this.policyRegisterForm.get("BDOId").value != "" && this.policyRegisterForm.get("BDOId").value != null && this.policyRegisterForm.get("BDOId").value != 0) {
      additionalFilters.push({ key: "BDOId", filterValues: [this.policyRegisterForm.get("BDOId").value] });
    }

    //BDM
    if (this.policyRegisterForm.get("BDMId").value != "" && this.policyRegisterForm.get("BDMId").value != null && this.policyRegisterForm.get("BDMId").value != 0) {
      additionalFilters.push({ key: "BDMId", filterValues: [this.policyRegisterForm.get("BDMId").value] });
    }
  ...
  // Build Main Form
  private _initForm(data: IPolicyRegisterReportDto) {
      ...
      .subscribe(res => {
        if (res.Success) {
          ...
          if (this.branches.length == 1) {
            this.policyRegisterForm.patchValue({ BranchId: this.branches[0].Id })
          }
        }
  ...
  private _onFormChange() {
    ...
    //Sales Person
    this.policyRegisterForm.get('SalesPersonName').valueChanges.subscribe((val) => {
      let Rule: IFilterRule[] = [{ Field: 'Branch.Id', Operator: 'eq', Value: this.policyRegisterForm.get('BranchId').value }];
      let AdditionalFilters: IAdditionalFilterObject[] = [{ key: "FullName", filterValues: [val] }];
    ...
    //Team Reference
    this.policyRegisterForm.get('TeamReferenceName').valueChanges.subscribe((val) => {
      let Rule: IFilterRule[] = [];
      let AdditionalFilters: IAdditionalFilterObject[] = [{ key: "FullName", filterValues: [val] }];
      AdditionalFilters.push({ key: 'UserType', filterValues: ['TeamReference'] });

      this.teamRefUser$ = this._masterListService.getFilteredMultiRulMasterDataList(API_ENDPOINTS.User.List, 'FirstName', "", Rule, AdditionalFilters)
        .pipe(takeUntil(this._destroy$), switchMap((res) => {
          if (res.Success) {
            if (res.Data.Items.length) {
              return of(res.Data.Items);
    ...
    //Group Head Name
    this.policyRegisterForm.get('GroupHeadName').valueChanges.subscribe((val) => {
      let Rule: IFilterRule[] = [{ Field: "Branch.Id", Operator: "eq", Value: this.policyRegisterForm.get('BranchId').value }];
    ...
    //BDO
    this.policyRegisterForm.get('BDOName').valueChanges.subscribe((val) => {
      let ActiveMasterData: IFilterRule[] = [{ Field: 'Branch.Id', Operator: 'eq', Value: this.policyRegisterForm.get('BranchId').value, }];
    ...
    //BDM
    this.policyRegisterForm.get('BDMName').valueChanges.subscribe((val) => {
      let ActiveMasterData: IFilterRule[] = [{ Field: 'Branch.Id', Operator: 'eq', Value: this.policyRegisterForm.get('BranchId').value, }];
