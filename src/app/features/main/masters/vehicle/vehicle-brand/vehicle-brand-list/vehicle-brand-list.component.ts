...
@Component({
  selector: 'gnx-vehicle-brand-list',
  templateUrl: './vehicle-brand-list.component.html',
  styleUrls: ['./vehicle-brand-list.component.scss'],
})
export class VehicleBrandListComponent {
  title: string;

  destroy$: Subject<any>;
  BrandList: BehaviorSubject<IVehicleBrandDto>;
  BrandList$: Observable<any>;
  api = API_ENDPOINTS.VehicleBrand.Base;
      ...
      listActions: [
        {
          ...
          tooltip: "View",
          icon: 'fa fa-eye',
          action: (item) => { this._router.navigate([ROUTING_PATH.Master.Vehicle.VBrandView + '/' + item.Id]) },
          hidden: (item) => (this._authService._userProfile.value?.AuthKeys?.includes("VehicleBrand-get"))
        },
        {
          name: "Edit",
          tooltip: "Edit",
          icon: 'fa fa-edit',
          action: (item) => { this._router.navigate([ROUTING_PATH.Master.Vehicle.VBrandEdit + '/' + item.Id]) },
          hidden: (item) => (this._authService._userProfile.value?.AuthKeys?.includes("VehicleBrand-update"))
        },
        {
          name: "Delete",
          tooltip: "Delete",
          icon: 'fa fa-trash',
          action: (item) => this._deleteBrand(item.Id),
          hidden: (item) => (this._authService._userProfile.value?.AuthKeys?.includes("VehicleBrand-delete"))
        },
      ]
    },
  ];
  ...
  constructor(
    ...
    this.BrandList = new BehaviorSubject(null);
    this.BrandList$ = this.BrandList.asObservable();
    this._init();
  }
...
*/
  get canDisplayCreateBtn(): boolean {
    if (this._authService._userProfile?.value?.AuthKeys?.includes("VehicleBrand-create")) {
      return true;
    } else {
      return false;
    }
