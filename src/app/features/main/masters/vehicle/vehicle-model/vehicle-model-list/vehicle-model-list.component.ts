...
@Component({
  selector: 'gnx-vehicle-model-list',
...
export class VehicleModelListComponent {
  ...
  destroy$: Subject<any>;
  ModelList: BehaviorSubject<IVehicleModelDto>;
  ModelList$: Observable<any>;
  api = API_ENDPOINTS.VehicleModel.Base;
  ...
  columnDef: AppDataGridListDto[] = [
    ...
    },
    {
      ...
      oprator: 'contains',
      isSortable: true,
      sortFieldName: 'Brand.Name',
      isFilterable: true,
      searchFieldName: 'Brand.Name',
      filterType: 'text',
      width: '30%',
      minWidth: '150px',
    },
      ...
      listActions: [
        {
          ...
          tooltip: "View",
          icon: 'fa fa-eye',
          action: (item) => { this._router.navigate([ROUTING_PATH.Master.Vehicle.VModelView + '/' + item.Id]) },
          hidden: (item) => (this._authService._userProfile.value?.AuthKeys?.includes("VehicleModel-get"))
        },
        {
          name: "Edit",
          tooltip: "Edit",
          icon: 'fa fa-edit',
          action: (item) => { this._router.navigate([ROUTING_PATH.Master.Vehicle.VModelEdit + '/' + item.Id]) },
          hidden: (item) => (this._authService._userProfile.value?.AuthKeys?.includes("VehicleModel-update"))
        },
        {
          name: "Delete",
          tooltip: "Delete",
          icon: 'fa fa-trash',
          action: (item) => this._deleteModel(item.Id),
          hidden: (item) => (this._authService._userProfile.value?.AuthKeys?.includes("VehicleModel-delete"))
        },
      ]
    },
  ];
  ...
  constructor(
    ...
    this.ModelList = new BehaviorSubject(null);
    this.ModelList$ = this.ModelList.asObservable();
    this._init();
  }
...
*/
  get canDisplayCreateBtn(): boolean {
    if (this._authService._userProfile?.value?.AuthKeys?.includes("VehicleModel-create")) {
      return true;
    } else {
      return false;
    }
...
*/
  get canDisplayExportBtn(): boolean {
    if (this._authService._userProfile?.value?.AuthKeys?.includes("VehicleModel-export")) {
      return true;
    } else {
      return false;
    }
...
this.pagefilters.currentPage = 1;
  ...
  public downloadMasterDataExcel() {
    ...
    this._dataService.exportToExcel(spec, API_ENDPOINTS.VehicleModel.Export).subscribe((blob) => {
