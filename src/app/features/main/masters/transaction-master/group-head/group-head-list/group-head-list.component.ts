...
@Component({
  selector: 'gnx-group-head-list',
  templateUrl: './group-head-list.component.html',
  styleUrls: ['./group-head-list.component.scss']
})
export class GroupHeadListComponent implements OnInit, OnDestroy {
  // #region public variables

  title: string;
  destroy$: Subject<any>;

  GroupHeadList: BehaviorSubject<any>;
  GroupHeadList$: Observable<any>;
  ...
  columnDef: AppDataGridListDto[] = [
    {
      head: 'Group Head ID',
      fieldName: 'GroupHeadNumber',
      oprator: 'contains',
      isSortable: true,
      sortFieldName: 'GroupHeadNumber',
      isFilterable: true,
      searchFieldName: 'GroupHeadNumber',
      filterType: 'text',
      width: '15%',
      minWidth: '150px',
    },
    ...
    {
      head: 'Associated POSP',
      fieldName: 'SalesPersonName',
      oprator: 'contains',
      isSortable: true,
      sortFieldName: 'SalesPerson',
      isFilterable: true,
      searchFieldName: 'SalesPerson',
      filterType: 'text',
      width: '30%',
      minWidth: '150px',
      isAdditional: true,
    ...
    {
      head: 'Reference User',
      fieldName: 'TeamReferenceUserName',
      oprator: 'contains',
      isSortable: true,
      sortFieldName: 'TeamReferenceUser',
      isFilterable: true,
      searchFieldName: 'TeamReferenceUser',
      filterType: 'text',
      width: '30%',
      minWidth: '150px',
      isAdditional: true,
      ...
      listActions: [
        {
          ...
          tooltip: "View",
          icon: 'fa fa-eye',
          action: (item) => { this._router.navigate([ROUTING_PATH.Master.Transaction.groupHeadView + '/' + item.Id]) },
          hidden: (item) => (this._authService._userProfile.value?.AuthKeys?.includes("GroupHead-get"))
        },
        {
          name: "Edit",
          tooltip: "Edit",
          icon: 'fa fa-edit',
          action: (item) => { this._router.navigate([ROUTING_PATH.Master.Transaction.groupHeadEdit + '/' + item.Id]) },
          hidden: (item) => (this._authService._userProfile.value?.AuthKeys?.includes("GroupHead-update"))
        },
        {
          name: "Delete",
          tooltip: "Delete",
          icon: 'fa fa-trash',
          action: (item) => this.deleteGroupHead(item.Id),
          hidden: (item) => (this._authService._userProfile.value?.AuthKeys?.includes("GroupHead-delete"))
        },
      ]
    },
  ];
  ...
  constructor(
    ...
    this.GroupHeadList = new BehaviorSubject(null);
    this.GroupHeadList$ = this.GroupHeadList.asObservable();
  }
...
*/
  get canDisplayCreateBtn(): boolean {
    if (this._authService._userProfile?.value?.AuthKeys?.includes("GroupHead-create")) {
      return true;
    } else {
      return false;
    }
...
*/
  get canDisplayExportBtn(): boolean {
    if (this._authService._userProfile?.value?.AuthKeys?.includes("GroupHead-export")) {
      return true;
    } else {
      return false;
    }
  ...
  public deleteGroupHead(id) {
    this._dialogService
      ...
      .subscribe((res) => {
        if (res) {
          this._dataService.deleteData(id, this.GroupHeadListApi).subscribe((res) => {
            if (res.Success) {
  ...
  public downloadMasterDataExcel() {
    ...
    spec.OrderBySpecs = [{field: 'GroupHeadNumber',direction: 'desc'},];

    this._dataService.exportToExcel(spec, API_ENDPOINTS.GroupHead.Export).subscribe((blob) => {
  ...
  private _loadLists() {
    ...
    this._dataService
      .getDataList(listRequestBody, this.GroupHeadListApi)
      .pipe(takeUntil(this.destroy$))
      .subscribe(
        (res) => {
          if (res.Success) {
            this.GroupHeadList.next(res);
          }
        },
        (err) => {
        }
