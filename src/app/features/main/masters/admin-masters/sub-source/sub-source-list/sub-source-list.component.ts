import { Component } from '@angular/core';
import { ActivatedRoute, Router } from '@angular/router';
import { DialogService } from '@lib/services/dialog.service';
import { AlertsService } from '@lib/services/error-handling/alerts.service';
import { BehaviorSubject, Observable, Subject, takeUntil } from 'rxjs';
import { SubsourceService } from '../subsource.service';
import { API_ENDPOINTS } from '@config/api-endpoints.config';
import { HttpService } from '@lib/services/http/http.service';
import { ISubSourceDto } from '@models/dtos/core/SubSourceDto';
import { ColumnSearchService } from '@lib/services/columnSearch/column-search.service';
import { AuthService } from '@services/auth/auth.service';
import { AppDataGridListDto, YesNoFormatter } from '@models/common';
import { ROUTING_PATH } from '@config/routingPath.config';

@Component({
  selector: 'gnx-sub-source-list',
  templateUrl: './sub-source-list.component.html',
  styleUrls: ['./sub-source-list.component.scss']
})
export class SubSourceListComponent {
  // #region public variables

  title: string;

  destroy$: Subject<any>;
  SubSourceList: BehaviorSubject<ISubSourceDto>;
  SubSourceList$: Observable<any>

  // Default page filters
  pagefilters = {
    currentPage: 1,
    limit: 20,
    columnSearchOptions: {
      field: 'Name',
      searchString: '',
      operator: '',
    },
    columnSortOptions: {
      orderField: 'Name',
      orderDirection: 'asc'
    }
  }


  // Column Defination of table
  columnDef: AppDataGridListDto[] = [
    {
      head: 'Sub Source',
      fieldName: 'Name',
      oprator: 'contains',
      isSortable: true,
      sortFieldName: 'Name',
      isFilterable: true,
      searchFieldName: 'Name',
      filterType: 'text',
      width: '40%',
      minWidth: '150px',
    },
    {
      head: 'Source',
      fieldName: 'SourceName',
      oprator: 'contains',
      isSortable: true,
      sortFieldName: 'Source.Name',
      isFilterable: true,
      searchFieldName: 'Source.Name',
      filterType: 'text',
      width: '40%',
      minWidth: '150px',
    },
    {
      head: 'Is Active?',
      fieldName: 'Status',
      oprator: 'eq',
      isSortable: true,
      sortFieldName: 'Status',
      isFilterable: true,
      searchFieldName: 'Status',
      filterType: 'dropdown',
      drpDataList: [
        { Text: '-- select --', Value: '-1' },
        { Text: 'Yes', Value: '1' },
        { Text: 'No', Value: '0' },
      ],
      width: '10%',
      minWidth: '150px',
      valueFormatter: YesNoFormatter
    },
    {
      head: 'Actions',
      fieldName: '',
      listActions: [
        {
          name: "View",
          tooltip: "View",
          icon: 'fa fa-eye',
          action: (item) => { this._router.navigate([ROUTING_PATH.Master.Admin.SubSourceView + '/' + item.Id]) },
          hidden: (item) => (this._authService._userProfile.value?.AuthKeys?.includes("SubSource-get"))
        },
        {
          name: "Edit",
          tooltip: "Edit",
          icon: 'fa fa-edit',
          action: (item) => { this._router.navigate([ROUTING_PATH.Master.Admin.SubSourceEdit + '/' + item.Id]) },
          hidden: (item) => (this._authService._userProfile.value?.AuthKeys?.includes("SubSource-update"))
        },
        {
          name: "Delete",
          tooltip: "Delete",
          icon: 'fa fa-trash',
          action: (item) => this.deleteSubSource(item.Id),
          hidden: (item) => (this._authService._userProfile.value?.AuthKeys?.includes("SubSource-delete"))
        },
      ]
    },
  ];
  // #end region public variables

  /**
   * #region constructor
   * @param _route : used for getting dynamic route or id
   */

  constructor(private _route: ActivatedRoute, private _SubSourceService: SubsourceService,
   private _dataService:HttpService,
   private _alertservice: AlertsService,
    private _dialogService: DialogService,
    private _column: ColumnSearchService,
    private _router: Router,
    private _authService: AuthService,
    ) {

    this.destroy$ = new Subject();
    this._column.FilterConditions.Rules = [];
    this._column.AdditionalFilters = [];
    this._column.OrderBySpecs = [{field:'CreatedDate',direction:'desc'}];
    this.SubSourceList = new BehaviorSubject(null);
    this.SubSourceList$ = this.SubSourceList.asObservable();
    this._init()
  }

  // #endregion constructor

  /**
* If User Have Access for Buy Policy then Display Add button
*/
  get canDisplayCreateBtn(): boolean {
    if (this._authService._userProfile?.value?.AuthKeys?.includes("SubSource-create")) {
      return true;
    } else {
      return false;
    }
  }

  //#region public-methods
  // -----------------------------------------------------------------------------------------------------
  // @ Public methods
  // -----------------------------------------------------------------------------------------------------

  sortColumn(column: string) {
    this._column.UpdateSort(column);
this.pagefilters.currentPage = 1;
    this._loadSubSources()
  }

  searchColumn(value) {
    this._column.UpdateFilter(value);
    this.pagefilters.currentPage = 1
    this._loadSubSources();
  }

  setLimit(value) {
    this.pagefilters.limit = value
    this._loadSubSources()
  }

  // delete SubSource from list
  deleteSubSource(id) {
    this._dialogService.confirmDialog({
      title: 'Are You Sure?',
      message: "You won't be able to revert this",
      confirmText: 'Yes, Delete!',
      cancelText: "No"
    }).subscribe(res => {
      if (res) {
        this._SubSourceService.deleteSubSource(id).subscribe((res) => {
          if (res.Success) {
            this._alertservice.raiseSuccessAlert(res.Message, 'true')
            this._loadSubSources();
          }
          else{
            this._alertservice.raiseErrors(res.Alerts);
          }
        });
      }
    })
  }

  // pagination for next page
  nextPage() {
    this.pagefilters.currentPage = this.pagefilters.currentPage + 1;
    this._loadSubSources()
  }

  // pagination for prev page
  previousPage() {
    this.pagefilters.currentPage = this.pagefilters.currentPage - 1;
    this._loadSubSources()
  }

  // #endregion public methods

  //#region private-methods
  // -----------------------------------------------------------------------------------------------------
  // @ Private methods
  // -----------------------------------------------------------------------------------------------------

  private _init() {
    // Get Title from route
    this._route.data.subscribe((x: any) => {
      this.title = x.title
    })

    this._loadSubSources()
  }

  private _loadSubSources() {
    let listRequestBody = {
      IncludeDeleted: false,
      PaginationSpecs: {
        PaginationRequired: true,
        Page: this.pagefilters.currentPage,
        Limit: this.pagefilters.limit,
      },
      FilterConditions: this._column.FilterConditions,
      OrderBySpecs: this._column.OrderBySpecs,
      AdditionalFilters: this._column.AdditionalFilters,
      DisplayColumns: [],
    };

    let api=API_ENDPOINTS.SubSource.Base

    this._dataService
      .getDataList(listRequestBody, api)
      .pipe(takeUntil(this.destroy$))
      .subscribe(
        (res) => {
          if (res.Success) {
            this.SubSourceList.next(res);
          }
        },
        (err) => {
        }
      );

  }

  // #endregion private methods

}
