...
import { IEngineeringDocumentsDto, ISendBackRejectDTO, SendBackRejectDTO, IEngineeringPaymentLinkUWDto } from '@models/dtos';
import { environment } from 'src/environments/environment';
import { SubCategoryCodeEnum } from 'src/app/shared/enums';

@Component({
  selector: 'gnx-engineering-payment-link-uw',
  templateUrl: './engineering-payment-link-uw.component.html',
  styleUrls: ['./engineering-payment-link-uw.component.scss'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class EngineeringPaymentLinkUwComponent {
  ...
  get Documents() {
    return this.EngineeringPLWNForm.get('Documents') as FormArray;
  }
  ...
  ngOnInit(): void {
    ...
    // Init Form
    this.EngineeringPLWNForm = this._initForm(this.DisplayForm);
    ...
    if (this.DisplayForm.PaymentMode == 'Cheque') {
      this.EngineeringPLWNForm.get('PaymentInsurer').disable();
  ...
  // Reject Button 
  public RejectButton() {
    if (this.EngineeringPLWNForm.get('SendBackRejectDesc').value == "" || this.EngineeringPLWNForm.get('SendBackRejectDesc').value == null) {
    ...
    this._dialogService
      ...
      .subscribe((res) => {
        if (res) {
          ...
          SendBackRejectObj.Id = this.EngineeringPLWNForm.value.Id;
          SendBackRejectObj.Stage = this.EngineeringPLWNForm.value.Stage;
          SendBackRejectObj.SendBackRejectDesc = this.EngineeringPLWNForm.value.SendBackRejectDesc;
  ...
  public SendBackButton() {
    ...
    this._dialogService
      ...
      .subscribe((res) => {
        if (res) {
          let Id = this.EngineeringPLWNForm.get('Id').value
  ...
  public SubmitFormButton() {
    ...
    let submitFormValue = JSON.parse(JSON.stringify(this.EngineeringPLWNForm.value))

    if (!submitFormValue.ProposalSubmissionDetail.StorageFilePath) {
      submitFormValue.ProposalSubmissionDetail = null
    }

    this._rfqEngineeringService.SubmitPaymentLink(submitFormValue).subscribe(res => {
      if (res.Success) {
        this._alertservice.raiseSuccessAlert(res.Message, "false")
        this._router.navigate([ROUTING_PATH.Basic.Dashboard])
      }
      ...
      this._dataService
        ...
        .subscribe((res) => {
          if (res.Success) {
            ...
            this.EngineeringPLWNForm.get('ProposalSubmissionDetail').patchValue({
  ...
  private _initForm(data: IEngineeringPaymentLinkUWDto) {
