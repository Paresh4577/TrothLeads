...
import { EngineeringPolicyType, EngineeringRenewalPolicyType } from '@config/rfq';
...
import { EngineeringRaiseDTO, IEngineeringRaiseDTO } from '@models/dtos';
import { EngineeringDocumentsDto } from '@models/dtos';
...
@Component({
  selector: 'gnx-engineering-raise',
  templateUrl: './engineering-raise.component.html',
  styleUrls: ['./engineering-raise.component.scss'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class EngineeringRaiseComponent {
  ...
  RFQEngineering: IEngineeringRaiseDTO
  destroy$: Subject<any>;
  ...
  ngOnInit(): void {
    ...
    this.RFQEngineering = new EngineeringRaiseDTO();
  ...
  // get Engineering policy type
  get EngineeringPolicyType() {

    if (this.DisplayForm?.TransactionId) {
      return EngineeringRenewalPolicyType;
    }
    else {
      return EngineeringPolicyType;
    }
  }
 ...
 */
  ...
  */
  ...
  public ProductCategoryDetailsValidations() {
    ...
    if (!this.RFQEngineeringForm.get('RiskLocation').value) {
      this.ProductCategoryDetailsAlert.push({
  ...
  */
  public addDocuments(selectedDocument?: string) {
    const row: EngineeringDocumentsDto = new EngineeringDocumentsDto();
...
*/
  ...
  public SubmitRfqEngineering() {
    ...
    switch (this.mode) {
      case "create": case "RenewalRFQ":
        this._RFQEngineeringService.CreateProposal(this.RFQEngineeringForm.value).subscribe((res) => {
          if (res.Success) {
      ...
      case "edit":
        this._RFQEngineeringService.UpdateProposal(this.RFQEngineeringForm.value).subscribe((res) => {
          if (res.Success) {
            this._alertservice.raiseSuccessAlert(res.Message, "false")
            this._router.navigate([ROUTING_PATH.Basic.Dashboard])
          }
  ...
  private _buildForm(data: IEngineeringRaiseDTO) {
  ...
  private _buildDocumentsForm(items: EngineeringDocumentsDto[] = []): FormArray {
  ...
  private _initDocumentsForm(item: EngineeringDocumentsDto): FormGroup {
  ...
  private _onFormChange() {
    ...
    this.RFQEngineeringForm.get('SubCategoryId').valueChanges.subscribe(val => {
      ...
      if (SelectedSubCategory.Code == SubCategoryCodeEnum.CPM || SelectedSubCategory.Code == SubCategoryCodeEnum.EEIP) {
        this.RFQEngineeringForm.patchValue({
          ProjectDetails: ""
        });
      }
  ...
  private _fillMasterList() {
    ...
    let SubCategoryRule: IFilterRule[] = [ActiveMasterDataRule,
      { Field: "Category.Code", Operator: "eq", Value: CategoryCodeEnum.Engineering }
    ]
    ...
    let InsuranceCompanyAdditionalFilters: IAdditionalFilterObject[] = [{ key: "CatagoryCode", filterValues: [CategoryCodeEnum.Engineering] }]
