...
@Component({
  selector: 'gnx-engineering-rfq-list',
  templateUrl: './engineering-rfq-list.component.html',
  styleUrls: ['./engineering-rfq-list.component.scss']
})
export class EngineeringRfqListComponent {
  ...
  EngineeringRfqList: BehaviorSubject<any>; //Store RFQ list
  EngineeringRfqList$: Observable<any>;  // Observable for RFQ lits
  ...
  ngOnInit(): void {
    ...
    this.EngineeringRfqList = new BehaviorSubject(null);
    this.EngineeringRfqList$ = this.EngineeringRfqList.asObservable();
...
*/
  ...
  // back button
  public addButton() {
    this._router.navigate([ROUTING_PATH.RFQEngineering.EngineeringRaise])
  }
  ...
  private _loadLists() {
    ...
    this._dataService
      ...
      .subscribe(
        (res) => {
          if (res.Success) {
            this.EngineeringRfqList.next(res);
          }
        },
        (err) => {
        }
