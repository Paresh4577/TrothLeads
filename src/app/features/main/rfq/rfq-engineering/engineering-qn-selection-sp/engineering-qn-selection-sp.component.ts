...
import { EngineeringDocumentsDto, IEngineeringDocumentsDto, IEngineeringQNDocumentsDto, ISendBackRejectDTO, SendBackRejectDTO } from '@models/dtos';
import { environment } from 'src/environments/environment';
import { RfqService } from '../../rfq.service';

@Component({
  selector: 'gnx-engineering-qn-selection-sp',
  templateUrl: './engineering-qn-selection-sp.component.html',
  styleUrls: ['./engineering-qn-selection-sp.component.scss'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class EngineeringQnSelectionSpComponent {
  ...
  get QNselection() {
    return this.EngineeringQNUWForm.get('QNDocuments') as FormArray;
  }

  // Documents Form array
  get Documents() {
    return this.EngineeringQNUWForm.get('Documents') as FormArray;
  }
  ...
  get canKycDetailMandatory(): boolean {
    if (this.DisplayForm.SubCategoryCode == SubCategoryCodeEnum.ERA ||
      this.DisplayForm.SubCategoryCode == SubCategoryCodeEnum.CPM ||
      this.DisplayForm.SubCategoryCode == SubCategoryCodeEnum.EEIP ||
      this.DisplayForm.SubCategoryCode == SubCategoryCodeEnum.CRA) {
      return false;
    } else {
      return true;
  ...
  ngOnInit(): void {
    ...
    this.EngineeringQNUWForm = this._buildForm(this.DisplayForm);
  ...
  // Reject Button
  public RejectButton() {
    if (this.EngineeringQNUWForm.get('SendBackRejectDesc').value == "" || this.EngineeringQNUWForm.get('SendBackRejectDesc').value == null) {
    ...
    this._dialogService
      ...
      .subscribe((res) => {
        if (res) {
          ...
          SendBackRejectObj.Id = this.EngineeringQNUWForm.value.Id;
          SendBackRejectObj.Stage = this.EngineeringQNUWForm.value.Stage;
          SendBackRejectObj.SendBackRejectDesc = this.EngineeringQNUWForm.value.SendBackRejectDesc;
  ...
  public SubmitForm() {
    ...
    let SubmitFormValue = JSON.parse(JSON.stringify(this.EngineeringQNUWForm.value))
    ...
    this._RFQEngineeringService.SubmitQNSelectionSP(SubmitFormValue).subscribe((res) => {
      if (res.Success) {
        this._alertservice.raiseSuccessAlert(res.Message, "false")
        this._router.navigate([ROUTING_PATH.Basic.Dashboard])
      }
...
*/
  public SelectBuy(index: number) {
    ...
    this.QNselection.controls.forEach((qn, i) => {
      if (i == index) {
        ...
        if (this.EngineeringQNUWForm.get('PaymentMode').value == 'Cheque') {
          this.EngineeringQNUWForm.get('PaymentAccountName').patchValue(qn.value.InsuranceCompanyName)
        }
      } else {
  ...
  */
  public DonloadQNdocument() {
    ...
    if (SelectedQuotation) {
      this._RFQEngineeringService.DownloadQnDocument(SelectedQuotation.Id).subscribe(blob => {
  ...
  */
  public KYCDetailsValidationControl() {
    ...
    if (this.mode != "view") {
      ...
      if (!this.EngineeringQNUWForm.get('AadharNo').value) {
        if(this.canKycDetailMandatory){
      ...
      } else if (!this.UIDNumValidationReg.test(this.EngineeringQNUWForm.get('AadharNo').value)) {
        this.KYCDetailsAlerts.push({
      ...
      if (!this.EngineeringQNUWForm.get('PANNo').value) {
        if (this.canKycDetailMandatory) {
        this.KYCDetailsAlerts.push({
          Message: 'Enter  PAN',
          CanDismiss: false,
      ...
      } else if (!this.PANNumValidationReg.test(this.EngineeringQNUWForm.get('PANNo').value)) {
        this.KYCDetailsAlerts.push({
  ...
  public PaymentModeDetailValidationControl() {
    ...
    if (this.mode != "view") {
      if (!this.EngineeringQNUWForm.get("PaymentMode").value) {
      ...
      if (this.EngineeringQNUWForm.get("PaymentMode").value == 'Cheque') {
        if (!this.EngineeringQNUWForm.get("PaymentAccountName").value) {
          this.PaymentModeDetailsAlerts.push({
  ...
  public SelectKycDoc(event, DocumentType: string) {
    ...
    if (file) {
      this._dataService.UploadFile(this.UploadFileAPI, file).subscribe((res) => {
        if (res.Success) {
          if (DocumentType == 'UID') {
            this.EngineeringQNUWForm.get('AadharDetails').patchValue({
              RFQId: this.EngineeringQNUWForm.value.Id,
              DocumentType: "UID",
              DocumentTypeName: "Aadhar Card",
              FileName: res.Data.FileName,
              StorageFileName: res.Data.StorageFileName,
              StorageFilePath: res.Data.StorageFilePath,
              Stage: this.EngineeringQNUWForm.value.Stage
            })
          }
          else if (DocumentType == 'PAN') {
            this.EngineeringQNUWForm.get('PANDetails').patchValue({
              RFQId: this.EngineeringQNUWForm.value.Id,
              DocumentType: "PAN",
            ...
            this.EngineeringQNUWForm.get('GSTDetails').patchValue({
              FileName: res.Data.FileName,
              StorageFileName: res.Data.StorageFileName,
              StorageFilePath: res.Data.StorageFilePath,
  ...
  */
  ...
  private _OnformChange() {
    ...
    this.EngineeringQNUWForm.get('PaymentMode').valueChanges.subscribe(val => {
      if (val == 'Cheque') {
        let selectedQn = this.QNselection.value.find(qn => qn.Buy == true)
        if (selectedQn) {
          this.EngineeringQNUWForm.get('PaymentAccountName').patchValue(selectedQn.InsuranceCompanyName)
        }
      } else {
        this.EngineeringQNUWForm.get('PaymentAccountName').patchValue(null)
      }
    })
