...
import { CategoryCodeEnum, SubCategoryCodeEnum, UserTypeEnum } from 'src/app/shared/enums';
...
import { IEngineeringPaymentDetailsDto, EngineeringPaymentDetailsDto } from '@models/dtos';
import { environment } from 'src/environments/environment';
const ActiveMasterDataRule: IFilterRule = { Field: 'Status', Operator: 'eq', Value: 1 }

@Component({
  selector: 'gnx-engineering-payment-proof-sp',
  templateUrl: './engineering-payment-proof-sp.component.html',
  styleUrls: ['./engineering-payment-proof-sp.component.scss'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class EngineeringPaymentProofSpComponent {
  ...
  // #region Getters
  get Documents() {
    return this.EngineeringPPForm.get('Documents') as FormArray;
  }


  get PaymentDetails() {
    return this.EngineeringPPForm.get('PaymentDetails') as FormArray
  }
  ...
  ngOnInit(): void {
    ...
    this.EngineeringPPForm = this._buildForm(this.DisplayForm);
 ...
 */
    if (this.IsPOSPUser) {
      if (!this.EngineeringPPForm.get('PremiumAmountPaidBy').value) {
        this.EngineeringPPForm.get('PremiumAmountPaidBy').patchValue('Customer')
      }
    }
  }
  ...
  // Reject Button 
  public RejectButton() {
    if (this.EngineeringPPForm.get('SendBackRejectDesc').value == "" || this.EngineeringPPForm.get('SendBackRejectDesc').value == null) {
    ...
    this._dialogService
      ...
      .subscribe((res) => {
        if (res) {
          ...
          SendBackRejectObj.Id = this.EngineeringPPForm.value.Id;
          SendBackRejectObj.Stage = this.EngineeringPPForm.value.Stage;
          SendBackRejectObj.SendBackRejectDesc = this.EngineeringPPForm.value.SendBackRejectDesc;
  ...
  public SubmitForm() {
    ...
    this._RFQEngineeringService.SubmitPaymentProof(this.EngineeringPPForm.value).subscribe(res => {
      if (res.Success) {
        this._alertservice.raiseSuccessAlert(res.Message, "false")
        this._router.navigate([ROUTING_PATH.Basic.Dashboard])
      }
  ...
  */
          ...
          if (res.Success) {
            ...
            if (DocIndex > 0) {
              this.Documents.controls[DocIndex].patchValue({
                ...
                FileName: res.Data.FileName,
                RFQId: this.EngineeringPPForm.get('Id').value
              })
            }
            else {
              ...
              row.RFQId = this.EngineeringPPForm.get('Id').value
  ...
  public addPaymentDetails() {
    ...
    if (this.AttachPaymentProofAlerts.length > 0) {
      ...
      var row: IEngineeringPaymentDetailsDto = new EngineeringPaymentDetailsDto()
      row.RFQId = this.EngineeringPPForm.get("Id").value;
  ...
  public AttachPaymentProofValidationControl() {
    ...
    if (this.EngineeringPPForm.get('PremiumAmountPaidBy').value === "" || this.EngineeringPPForm.get('PremiumAmountPaidBy').value == null) {
 ...
 */
  ...
  private _buildPaymentDetailsForm(items: IEngineeringPaymentDetailsDto[] = []): FormArray {
    let formArray: FormArray = new FormArray([]);
    if (items != null) {
      if (!(items && items.length > 0)) {
        return formArray;
  ...
  private _initPaymentDetailsForm(data: IEngineeringPaymentDetailsDto): FormGroup {
