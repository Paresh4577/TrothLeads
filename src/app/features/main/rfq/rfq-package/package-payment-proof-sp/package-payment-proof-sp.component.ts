...
import { IPackagePrePolicyDTO, IPackageSuminnsuredQuestionDTO, IRFQPackageDocumentsDto, IRFQPackageNomineeDetailDto, PackagePrePolicyDTO, RFQPackageDocumentsDto, RFQPackageNomineeDetailDto } from '@models/dtos';
import { IRFQPackagePaymentDetailsDto, RFQPackagePaymentDetailsDto } from '@models/dtos';
...
@Component({
  selector: 'gnx-package-payment-proof-sp',
  templateUrl: './package-payment-proof-sp.component.html',
  styleUrls: ['./package-payment-proof-sp.component.scss'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class PackagePaymentProofSpComponent {
 ...
 */
  ...
  public submitForm(): void {
    ...
    this._rfqPackageService.submitPaymentProof(this.paymentProofForm.value).subscribe(res => {
      if (res.Success) {
        this._alertservice.raiseSuccessAlert(res.Message, "false")
        this._router.navigate([ROUTING_PATH.Basic.Dashboard])
      }
  ...
  */
  ...
  public addPaymentDetails(): void {
    ...
    if (this._attachPaymentProofAlerts.length > 0) {
      ...
      var row: IRFQPackagePaymentDetailsDto = new RFQPackagePaymentDetailsDto()
      row.RFQId = this.paymentProofForm.get("Id").value;
 ...
 */
  ...
  // Documents FormGroup
  private _initDocumentForm(data: RFQPackageDocumentsDto): FormGroup {
  ...
  private _buildPaymentDetailsForm(items: IRFQPackagePaymentDetailsDto[] = []): FormArray {
    let formArray: FormArray = new FormArray([]);
    if (items != null) {
      if (!(items && items.length > 0)) {
        return formArray;
  ...
  private _initPaymentDetailsForm(data: IRFQPackagePaymentDetailsDto): FormGroup {
