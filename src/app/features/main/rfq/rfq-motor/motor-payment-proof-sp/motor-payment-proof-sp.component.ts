...
import { IDocumentsDto, IPaymentProofDto, IPaymentDetailsDto, DocumentsDto, PaymentDetailsDto } from '@models/dtos/config/RFQMotor';
...
@Component({
  selector: 'gnx-motor-payment-proof-sp',
  templateUrl: './motor-payment-proof-sp.component.html',
  styleUrls: ['./motor-payment-proof-sp.component.scss'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class MotorPaymentProofSpComponent {
  ...
  ngOnInit(): void {
    ...
    this.mode = data['mode']
    // Init Form
    this.PPSForm = this._initForm(this.DisplayForm);
    this.SelectedQNDoc = this.DisplayForm.QNDocuments.find(ele => ele.Buy == true)
   ...
   */
    if (this.IsPOSPUser) {
      if (!this.PPSForm.get('PremiumAmountPaidBy').value) {
        this.PPSForm.get('PremiumAmountPaidBy').patchValue('Customer')
      }
    }
  }
  ...
  //Documents FormArray
  get Documents() {
    return this.PPSForm.get('Documents') as FormArray;
  }

  // Payment Details FormArray
  get PaymentDetails() {
    return this.PPSForm.get('PaymentDetails') as FormArray;
  }
  ...
  // get SelectedQNDoc() {
  //   return this.DisplayForm.QNDocuments.find((f) => f.Buy == true)
  // }

  get ProposalDoc() {
    let Index = this.Documents.value.findIndex(ele => ele.DocumentType == 'Proposal')
    if (Index != -1) {
  ...
  public SubmitForm() {
    ...
    this._RfqMotorService.SubmitPaymentProof(this.PPSForm.value).subscribe(res => {
      if (res.Success) {
  ...
  public SendBackButton() {
    if (this.PPSForm.get('SendBackRejectDesc').value == "" || this.PPSForm.get('SendBackRejectDesc').value == null) {
    ...
    this._dialogService
      ...
      .subscribe((res) => {
        if (res) {
          ...
          SendBackRejectObj.Id = this.PPSForm.value.Id;
          SendBackRejectObj.Stage = this.PPSForm.value.Stage;
          SendBackRejectObj.SendBackRejectDesc = this.PPSForm.value.SendBackRejectDesc;
  ...
  */
  ...
  public addChequeDocuments(MainRow: number) {
    const row: IDocumentsDto = new DocumentsDto();
    row.RFQId = this.PPSForm.get("Id").value;
  ...
  private _initForm(data: IPaymentProofDto) {
