...
import { WCDocumentsDto, IWCDocumentsDto, IWCPaymentDetailsDto, WCPaymentDetailsDto } from '@models/dtos';
...
@Component({
  selector: 'gnx-wc-loading-payment-proof-sp',
  templateUrl: './wc-loading-payment-proof-sp.component.html',
  styleUrls: ['./wc-loading-payment-proof-sp.component.scss'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class WcLoadingPaymentProofSpComponent {
  ...
  IsPOSPorTeamRefUser: boolean = false;
  ...
  */
  ...
  public SubmitForm() {
    ...
    this._RFQWcService.SubmitLoadingPaymentProof(this.LoadingPaymentProofForm.value).subscribe(res => {
      if (res.Success) {
        this._alertservice.raiseSuccessAlert(res.Message, "false")
        this._router.navigate([ROUTING_PATH.Basic.Dashboard])
      }
...
*/
  ...
  public addPaymentDetails() {
    ...
    if (this.AttachPaymentProofAlerts.length > 0) {
      ...
      var row: IWCPaymentDetailsDto = new WCPaymentDetailsDto()
      row.RFQId = this.LoadingPaymentProofForm.get("Id").value;
  ...
  // Documents FormArray
  private _buildPaymentDetailsForm(items: IWCPaymentDetailsDto[] = []): FormArray {
    let formArray: FormArray = new FormArray([]);
    if (items != null) {
      if (!(items && items.length > 0)) {
        return formArray;
  ...
  // Documents FormGroup
  private _initPaymentDetailsForm(data: IWCPaymentDetailsDto): FormGroup {
