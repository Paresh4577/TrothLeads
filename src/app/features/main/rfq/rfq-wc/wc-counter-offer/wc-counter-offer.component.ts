...
import { IWCQueriesDto, IWCCounterOfferDto, WCQueriesDto } from '@models/dtos';
...
@Component({
  selector: 'gnx-wc-counter-offer',
  templateUrl: './wc-counter-offer.component.html',
  styleUrls: ['./wc-counter-offer.component.scss'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class WcCounterOfferComponent {
  ...
  */
  ...
  ngOnInit(): void {
    ...
    // Init Form
    this.CounterOfferForm = this._initForm(this.DisplayForm);
    ...
    this.authService.userProfile$.subscribe((user: IMyProfile) => {
      if (user) {
        ...
        if (user.UserType == UserTypeEnum.Agent || user.UserType == UserTypeEnum.TeamReference) {
  ...
  public SubmitFormButton(IsSubmit: boolean) {
    ...
    this._rfqWcService.SubmitCounterOfferInfo(this.CounterOfferForm.value, IsSubmit).subscribe((res) => {
      if (res.Success) {
  ...
  public addQueryDetails(QueryRaiseBy: string) {
    ...
    if (QueryRaiseBy == "Insurer") {
      ...
      if (this.InsurerQueryAlerts.length > 0) {
        ...
        var row: IWCQueriesDto = new WCQueriesDto()
    ...
    if (this.InsurerQueries.controls.length > 0) {
      this.InsurerQueries.controls.forEach((el, i) => {
        ...
        if (el.get('NextFollowUpDate').value === "" || el.get('NextFollowUpDate').value === null) {
          ...
          if (moment(el.get('NextFollowUpDate').value, 'yyyy-MM-dd').isBefore(moment(el.get('QueryDate').value, 'yyyy-MM-dd'))) {
            this.InsurerQueryAlerts.push({
  ...
  public SendBackButton() {
    ...
    this._dialogService
      ...
      .subscribe((res) => {
        if (res) {
          let Id = this.CounterOfferForm.get('Id').value
          let SendBackRejectObj: ISendBackRejectDTO = new SendBackRejectDTO()
          SendBackRejectObj.Id = this.CounterOfferForm.value.Id;
          SendBackRejectObj.Stage = this.CounterOfferForm.value.Stage;
          SendBackRejectObj.SendBackRejectDesc = this.CounterOfferForm.value.SendBackRejectDesc;
  ...
  // Init Form
  private _initForm(data: IWCCounterOfferDto) {
  ...
  // Queries Formarray
  private _buildQueriesForm(items: IWCQueriesDto[] = []): FormArray {
  ...
  private _initQueriesForm(item: IWCQueriesDto): FormGroup {
