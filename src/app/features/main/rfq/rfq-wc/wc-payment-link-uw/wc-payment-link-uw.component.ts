...
import { IWCDocumentsDto, IWcPaymentLinkUWDto } from '@models/dtos';
...
@Component({
  selector: 'gnx-wc-payment-link-uw',
  templateUrl: './wc-payment-link-uw.component.html',
  styleUrls: ['./wc-payment-link-uw.component.scss'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class WcPaymentLinkUwComponent {
  ...
  public SubmitForm() {
    ...
    this._RFQWCService.SubmitPaymentLink(submitFormValue).subscribe(res => {
      if (res.Success) {
        this._alertservice.raiseSuccessAlert(res.Message, "false")
        this._router.navigate([ROUTING_PATH.Basic.Dashboard])
      }
  ...
  private _initForm(data: IWcPaymentLinkUWDto) {
