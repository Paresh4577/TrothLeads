...
import { WCDocumentsDto, IWCDocumentsDto, IWCPolicyIssueDto } from '@models/dtos';
...
@Component({
  selector: 'gnx-wc-policy-issue-uw',
  templateUrl: './wc-policy-issue-uw.component.html',
  styleUrls: ['./wc-policy-issue-uw.component.scss'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class WcPolicyIssueUwComponent {
  ...
  */
  ...
  public SubmitFormButton() {
    ...
    this._rfqWcService.SubmitPolicyIssue(this.PolicyIssueForm.value).subscribe(res => {
      if (res.Success) {
        this._alertservice.raiseSuccessAlert(res.Message, "false")
        this._router.navigate([ROUTING_PATH.Basic.Dashboard])
      }
  ...
  */
  ...
  // Init Form
  private _initForm(data: IWCPolicyIssueDto) {
  ...
  // Document formgroup
  private _initDocumentsForm(item: IWCDocumentsDto): FormGroup {
