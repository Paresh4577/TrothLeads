...
import { ILoadingPaymentDetailsDto, ILoadingPaymentDocumentDto, ILoadingPaymentProofDocumentsDto, LoadingPaymentDetailsDto, LoadingPaymentDocumentDto, LoadingPaymentProofDocumentsDto } from '@models/dtos/config/RFQHealth/loading-payment-proof.dto';
...
import { LoadingPaymentProofService } from './loading-payment-proof.service';
...
@Component({
  selector: 'gnx-loading-payment-proof',
  templateUrl: './loading-payment-proof.component.html',
  styleUrls: ['./loading-payment-proof.component.scss'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class LoadingPaymentProofComponent {
  ...
  LoadingPaymentProofForm: FormGroup; // Loading Payment Proof Document Form
  DisplayForm: any;

  // Step Control
  step3 = new FormControl(); // Step Validation Control

  // Alert Array List
  Step3alerts: Alert[] = []; // Step wise Error alert Array
  ...
  constructor(
    ...
    private _paymentProofService: LoadingPaymentProofService,
  ...
  ngOnInit(): void {
    ...
    this.LoadingPaymentProofForm = this._buildLoadingPaymentLinkForm(FormData)
    this.LoadingPaymentProofForm.get("LoadingPaymentInsurer").disable();
    ...
    this.PolicyPersonsArray = this.LoadingPaymentProofForm.get('Members') as FormArray;
    this.memberDispalyDetails(this.LoadingPaymentProofForm.get('Members').value)
    this._genderOfSelfSpouseInArray()

    // in case of "View" mode then default document condition not execute
    if (this.mode != "view" && this.LoadingPaymentProofForm.get("LoadingPaymentDetails").value.length <= 0) {
    ...
    if (this.mode == "view") {
      this.LoadingPaymentProofForm.disable();
    }
  ...
  //get Health Proposal Type From Config file
  ...
  get QNDocuments() {
    return this.LoadingPaymentProofForm.get('QNDocuments') as FormArray
  }
  ...
  // back button (return to RFQ Listing Page)
  ...
  public members() {
    ...
    this.myMembers = [];
    if (
      this.LoadingPaymentProofForm.get('SelfCoverRequired').value == true &&
      this.LoadingPaymentProofForm.get('SelfGender').value == 'Male'
    ) {
      this.myMembers.push({ member: '/assets/icons/male.png', title: 'Self' });
    }
    if (
      this.LoadingPaymentProofForm.get('SelfCoverRequired').value == true &&
      this.LoadingPaymentProofForm.get('SelfGender').value == 'Female'
    ) {
      this.myMembers.push({ member: '/assets/icons/woman.png', title: 'Self' });
    }
    if (
      this.LoadingPaymentProofForm.get('SpouseCoverRequired').value == true &&
      this.LoadingPaymentProofForm.get('SpouseGender').value == 'Male'
    ) {
      this.myMembers.push({ member: '/assets/icons/male.png', title: 'Spouse' });
    }
    if (
      this.LoadingPaymentProofForm.get('SpouseCoverRequired').value == true &&
      this.LoadingPaymentProofForm.get('SpouseGender').value == 'Female'
    ) {
      this.myMembers.push({ member: '/assets/icons/woman.png', title: 'Spouse' });
    }
    if (
      this.LoadingPaymentProofForm.get('DaughterCoverRequired').value == true &&
      this.LoadingPaymentProofForm.get('noOfDaughter').value == 1
    ) {
    ...
    if (
      this.LoadingPaymentProofForm.get('DaughterCoverRequired').value == true &&
      this.LoadingPaymentProofForm.get('noOfDaughter').value > 2
    ) {
      this.myMembers.push({ member: '/assets/icons/girl.png', title: 'Daughter3' });
    }

    if (
      this.LoadingPaymentProofForm.get('SonCoverRequired').value == true &&
      this.LoadingPaymentProofForm.get('noOfSon').value == 1
    ) {
    ...
    if (
      this.LoadingPaymentProofForm.get('SonCoverRequired').value == true &&
      this.LoadingPaymentProofForm.get('noOfSon').value > 2
    ) {
      this.myMembers.push({ member: '/assets/icons/son.png', title: 'Son3' });
    }
    if (this.LoadingPaymentProofForm.get('MotherCoverRequired').value == true) {
      this.myMembers.push({ member: '/assets/icons/mother.png', title: 'Mother' });
    }
    if (this.LoadingPaymentProofForm.get('FatherCoverRequired').value == true) {
  ...
  public SetCover(member: string, answer) {
    let Answer = answer
    this.LoadingPaymentProofForm.patchValue({
...
*/
  public StepThreeValidationControl() {
    this.Step3alerts = [];

    if (this.mode != "view") {

      if (this.LoadingPaymentProofForm.get('LoadingPremiumAmountPaidBy').value === "" || this.LoadingPaymentProofForm.get('LoadingPremiumAmountPaidBy').value == null) {
      ...
      this.Payments.controls.forEach((el, i) => {
        ...
        if (this.LoadingPaymentProofForm.get('PaymentMode').value == 'Cheque') {
  ...
  */
  public StepThreeValidationError() {
    if (this.Step3alerts.length > 0) {
      this._alertservice.raiseErrors(this.Step3alerts);
    }
  }
  ...
  public addPaymentDetails() {
    ...
    if (this.ChequeAlerts.length > 0) {
      ...
      var row: LoadingPaymentDetailsDto = new LoadingPaymentDetailsDto()
      row.RFQId = this.LoadingPaymentProofForm.get("Id").value;
      row.Stage = "RFQLoadingPaymentProofSP"
  ...
  public addChequeDocuments(MainRow: number) {
    const row: ILoadingPaymentDocumentDto = new LoadingPaymentDocumentDto();
    row.RFQId = this.LoadingPaymentProofForm.get("Id").value;
    this.Payments.controls.forEach((element, index) => {
      if (index == MainRow) {
        (element.get('Documents') as FormArray).push(this._initPaymentDetailsDocumentForm(row))
      }
    });
  }
                  ...
                  (element.get('Documents') as FormArray).controls.forEach((el, i) => {
                    if (i == CurrentRow) {
                      el.patchValue({
                        ...
                        RFQId: this.LoadingPaymentProofForm.get("Id").value,
  ...
  public addDocuments(selectedDocument?: string) {
    ...
    const row: ILoadingPaymentProofDocumentsDto = new LoadingPaymentProofDocumentsDto();
  ...
  public SubmitPaymentProof() {
    ...
    this._paymentProofService.SubmitPaymentProof(this.LoadingPaymentProofForm.value).subscribe(res => {
      if (res.Success) {
    ...
    this._dialogService
      ...
      .subscribe((res) => {
        if (res) {
          ...
          this._paymentProofService.Reject(this.LoadingPaymentProofForm.value).subscribe((res) => {
            if (res.Success) {
              this._alertservice.raiseSuccessAlert(res.Message, "false")
              this._router.navigate([ROUTING_PATH.Basic.Dashboard])
            }
    ...
    this._dialogService
      ...
      .subscribe((res) => {
        if (res) {
          ...
          this._paymentProofService.SendBack(this.LoadingPaymentProofForm.value).subscribe((res) => {
            if (res.Success) {
              this._alertservice.raiseSuccessAlert(res.Message, "false")
              this._router.navigate([ROUTING_PATH.Basic.Dashboard])
            }
  ...
  private _genderOfSelfSpouseInArray() {
    ...
    if (this.LoadingPaymentProofForm.get('SelfGender').value == 'Male') {
  ...
  private _countDaughterSon(child) {
    if (child == 'Daughter') {
      this.LoadingPaymentProofForm.patchValue({
        noOfDaughter: this.LoadingPaymentProofForm.get('noOfDaughter').value + 1
      })
    }

    if (child == 'Son') {
      this.LoadingPaymentProofForm.patchValue({
        noOfSon: this.LoadingPaymentProofForm.get('noOfSon').value + 1
      })
    }
  }
  ...
  private _buildDocumentsForm(items: ILoadingPaymentProofDocumentsDto[] = []): FormArray {
    let formArray: FormArray = new FormArray([]);
    if (items != null) {
      if (!(items && items.length > 0)) {
        return formArray;
  ...
  // init Payment Proof Documents Form 
  private _initDocumentForm(data: ILoadingPaymentProofDocumentsDto): FormGroup {
  ...
  private _buildPaymentDetailsForm(items: ILoadingPaymentDetailsDto[] = []): FormArray {
    ...
    if (items != null) {
      ...
      if (items != null) {
        items.forEach((i) => {
          if (i.Stage == "RFQLoadingPaymentProofSP") {
  ...
  private _initPaymentDetailsForm(data: ILoadingPaymentDetailsDto): FormGroup {
    ...
    if (data) {
      if (data.Stage == "RFQLoadingPaymentProofSP") {
