...
import { HealthPolicyStatus, HealthPolicyStatusCode } from 'src/app/shared/enums/rfq-health';
import { CurrencyFormatterWithoutSymbol, dateFormatter, IAdditionalFilterObject, IFilterRule } from '@models/common';
...
export class RFQListComponent {
  ...
  title: string = 'Policy Register - health' // RFQ List page header title
      ...
      if (res.Success) {
        ...
        if (res.Data.StageName == HealthPolicyStatus.RFQRaised) {
          ...
          if (this.StageCode != "" && this.StageCode != null && this.StageCode != undefined) {
            this._router.navigate([ROUTING_PATH.RFQ.Raise + Id + "?StageCode=" + StageCode])
          }
          else {
