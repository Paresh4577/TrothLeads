...
@Component({
  selector: 'gnx-marine-policy-issue-uw',
  templateUrl: './marine-policy-issue-uw.component.html',
  styleUrls: ['./marine-policy-issue-uw.component.scss'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class MarinePolicyIssueUwComponent {
  ...
  public SubmitForm() {
    ...
    this._RFQMarineService.SubmitPolicyIssue(this.PolicyIssueForm.value).subscribe(res => {
      if (res.Success) {
        this._alertservice.raiseSuccessAlert(res.Message, "false")
        this._router.navigate([ROUTING_PATH.Basic.Dashboard])
      }
