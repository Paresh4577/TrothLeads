...
import { MarineDocumentsDto, IMarineDocumentsDto, IMarineQNDocumentsDto } from '@models/dtos';
...
@Component({
  selector: 'gnx-marine-qn-selection-sp',
  templateUrl: './marine-qn-selection-sp.component.html',
  styleUrls: ['./marine-qn-selection-sp.component.scss'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class MarineQnSelectionSpComponent {
  ...
  get QNselection() {
    return this.MarineQNUWForm.get('QNDocuments') as FormArray;
  }

  // Documents Form array
  get Documents() {
    return this.MarineQNUWForm.get('Documents') as FormArray;
  }
  ...
  ngOnInit(): void {
    ...
    // build Marine form
    this.MarineQNUWForm = this._buildForm(this.DisplayForm);
  ...
  // Reject Button
  public RejectButton() {
    if (this.MarineQNUWForm.get('SendBackRejectDesc').value == "" || this.MarineQNUWForm.get('SendBackRejectDesc').value == null) {
    ...
    this._dialogService
      ...
      .subscribe((res) => {
        if (res) {
          ...
          SendBackRejectObj.Id = this.MarineQNUWForm.value.Id;
          SendBackRejectObj.Stage = this.MarineQNUWForm.value.Stage;
          SendBackRejectObj.SendBackRejectDesc = this.MarineQNUWForm.value.SendBackRejectDesc;
  ...
  public SubmitForm() {
    ...
    let SubmitFormValue = JSON.parse(JSON.stringify(this.MarineQNUWForm.value))
    ...
    this._RFQMarineService.SubmitQNSelectionSP(SubmitFormValue).subscribe((res) => {
      if (res.Success) {
        this._alertservice.raiseSuccessAlert(res.Message, "false")
        this._router.navigate([ROUTING_PATH.Basic.Dashboard])
      }
  ...
  public SelectBuy(index: number) {
    ...
    this.QNselection.controls.forEach((qn, i) => {
      if (i == index) {
        ...
        if (this.MarineQNUWForm.get('PaymentMode').value == 'Cheque') {
          this.MarineQNUWForm.get('PaymentAccountName').patchValue(qn.value.InsuranceCompanyName)
        }
      } else {
...
*/
  public DonloadQNdocument() {
    ...
    if (SelectedQuotation) {
      this._RFQMarineService.DownloadQnDocument(SelectedQuotation.Id).subscribe(blob => {
...
*/
  public KYCDetailsValidationControl() {
    ...
    if (this.mode != "view") {
      ...
      if (!this.MarineQNUWForm.get('AadharNo').value) {
        /**
        ...
        //   AutoClose: false,
        // });
      } else if (!this.UIDNumValidationReg.test(this.MarineQNUWForm.get('AadharNo').value)) {
        this.KYCDetailsAlerts.push({
      ...
      if (!this.MarineQNUWForm.get('PANNo').value) {
        this.KYCDetailsAlerts.push({
          Message: 'Enter  PAN',
          CanDismiss: false,
          AutoClose: false,
        });
      } else if (!this.PANNumValidationReg.test(this.MarineQNUWForm.get('PANNo').value)) {
        this.KYCDetailsAlerts.push({
  ...
  public PaymentModeDetailValidationControl() {
    ...
    if (this.mode != "view") {
      if (!this.MarineQNUWForm.get("PaymentMode").value) {
      ...
      if (this.MarineQNUWForm.get("PaymentMode").value == 'Cheque') {
        if (!this.MarineQNUWForm.get("PaymentAccountName").value) {
          this.PaymentModeDetailsAlerts.push({
  ...
  public SelectKycDoc(event, DocumentType: string) {
    ...
    if (file) {
      this._dataService.UploadFile(this.UploadFileAPI, file).subscribe((res) => {
        if (res.Success) {
          if (DocumentType == 'UID') {
            this.MarineQNUWForm.get('AadharDetails').patchValue({
              RFQId: this.MarineQNUWForm.value.Id,
              DocumentType: "UID",
              DocumentTypeName: "Aadhar Card",
              FileName: res.Data.FileName,
              StorageFileName: res.Data.StorageFileName,
              StorageFilePath: res.Data.StorageFilePath,
              Stage: this.MarineQNUWForm.value.Stage
            })
          }
          else if (DocumentType == 'PAN') {
            this.MarineQNUWForm.get('PANDetails').patchValue({
              RFQId: this.MarineQNUWForm.value.Id,
              DocumentType: "PAN",
  ...
  */
  ...
  private _OnformChange() {
    ...
    this.MarineQNUWForm.get('PaymentMode').valueChanges.subscribe(val => {
      if (val == 'Cheque') {
        let selectedQn = this.QNselection.value.find(qn => qn.Buy == true)
        if (selectedQn) {
          this.MarineQNUWForm.get('PaymentAccountName').patchValue(selectedQn.InsuranceCompanyName)
        }
      } else {
        this.MarineQNUWForm.get('PaymentAccountName').patchValue(null)
      }
    })
