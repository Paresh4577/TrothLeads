...
import { IMarineQueriesDto, MarineQueriesDto } from '@models/dtos';
import { environment } from 'src/environments/environment';
import * as moment from 'moment';
import { ISendBackRejectDTO, SendBackRejectDTO } from '@models/dtos/config/rfq-common';

@Component({
  selector: 'gnx-marine-counter-offer',
  templateUrl: './marine-counter-offer.component.html',
  styleUrls: ['./marine-counter-offer.component.scss'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class MarineCounterOfferComponent {
  ...
  public SubmitForm(IsSubmit: boolean) {
    ...
    this._RFQMarineService.SubmitCounterOfferInfo(this.CounterOfferForm.value, IsSubmit).subscribe((res) => {
      if (res.Success) {
        this._alertservice.raiseSuccessAlert(res.Message, "false")
        this._router.navigate([ROUTING_PATH.Basic.Dashboard])
      }
  ...
  public addQueryDetails(QueryRaiseBy: string) {
    ...
    if (QueryRaiseBy == "Insurer") {
      ...
      if (this.InsurerQueryAlerts.length > 0) {
        ...
        var row: IMarineQueriesDto = new MarineQueriesDto()
      ...
      this.InsurerQueries.controls.forEach((el, i) => {
        if (el.get('QueryDate').value === "" || el.get('QueryDate').value === null) {
          this.InsurerQueryAlerts.push({
            Message: `${el.get('SrNo').value}. Query Date is required (Insurance company).`,
            CanDismiss: false,
            AutoClose: false,
          })
        } else {
        ...
        if (el.get('NextFollowUpDate').value === "" || el.get('NextFollowUpDate').value === null) {
          // this.InsurerQueryAlerts.push({
          //   Message: `${el.get('SrNo').value}. Next Follow up Date is required (Insurance company).`,
          //   CanDismiss: false,
          //   AutoClose: false,
          // })
        } else {
          if (moment(this._datePipe.transform(el.get('QueryDate').value, 'yyyy-MM-dd')).isSameOrBefore(moment(this._datePipe.transform(el.get('NextFollowUpDate').value, 'yyyy-MM-dd'))) == false) {
            this.InsurerQueryAlerts.push({
              Message: `${el.get('SrNo').value}. Next Follow up Date must be same or greater than Query Date (Insurance company).`,
              CanDismiss: false,
              AutoClose: false,
            })
          }
      ...
      this.CustomerQueries.controls.forEach((el, i) => {
        if (el.get('QueryDate').value === "" || el.get('QueryDate').value === null) {
          this.CustomerQueryAlerts.push({
            Message: `${el.get('SrNo').value}. Query Date is required (Customer's Response).`,
            CanDismiss: false,
            AutoClose: false,
          })
        } else {
        ...
        if (el.get('NextFollowUpDate').value === "" || el.get('NextFollowUpDate').value === null) {
          // this.CustomerQueryAlerts.push({
          //   Message: `${el.get('SrNo').value}. Next Follow up Date is required (Customer's Response).`,
          //   CanDismiss: false,
          //   AutoClose: false,
          // })
        } else {
          if (moment(this._datePipe.transform(el.get('QueryDate').value, 'yyyy-MM-dd')).isSameOrBefore(moment(this._datePipe.transform(el.get('NextFollowUpDate').value, 'yyyy-MM-dd'))) == false) {
            this.CustomerQueryAlerts.push({
              Message: `${el.get('SrNo').value}. Next Follow up Date must be same or greater than Query Date (Customer's Response).`,
              CanDismiss: false,
              AutoClose: false,
            })
          }
...
*/
  ...
  // Queries Formarray
  private _buildQueriesForm(items: IMarineQueriesDto[] = []): FormArray {
  ...
  private _initQueriesForm(item: IMarineQueriesDto): FormGroup {
