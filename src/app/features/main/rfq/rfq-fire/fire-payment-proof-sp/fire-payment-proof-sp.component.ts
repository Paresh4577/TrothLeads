...
import { FireDocumentsDto, IFireDocumentsDto, IFirePaymentDetailsDto, FirePaymentDetailsDto } from '@models/dtos';
...
@Component({
  selector: 'gnx-fire-payment-proof-sp',
  templateUrl: './fire-payment-proof-sp.component.html',
  styleUrls: ['./fire-payment-proof-sp.component.scss'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class FirePaymentProofSpComponent {
 ...
 */
  ...
  public SubmitForm() {
    ...
    this._RfqFireService.SubmitPaymentProof(this.PaymentProofForm.value).subscribe(res => {
      if (res.Success) {
        this._alertservice.raiseSuccessAlert(res.Message, "false")
        this._router.navigate([ROUTING_PATH.Basic.Dashboard])
      }
  ...
  */
  ...
  public addPaymentDetails() {
    ...
    if (this.AttachPaymentProofArrayAlerts.length > 0) {
      ...
      var row: IFirePaymentDetailsDto = new FirePaymentDetailsDto()
      row.RFQId = this.PaymentProofForm.get("Id").value;
  ...
  */
  ...
  private _buildPaymentDetailsForm(items: IFirePaymentDetailsDto[] = []): FormArray {
    let formArray: FormArray = new FormArray([]);
    if (items != null) {
      if (!(items && items.length > 0)) {
        return formArray;
  ...
  private _initPaymentDetailsForm(data: IFirePaymentDetailsDto): FormGroup {
