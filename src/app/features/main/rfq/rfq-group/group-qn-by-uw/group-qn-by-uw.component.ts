...
import { IGroupQNbyUWDTO } from '@models/dtos';
import { AuthService } from '@services/auth/auth.service';
import { CategoryCodeEnum } from 'src/app/shared/enums';
import { environment } from 'src/environments/environment';
import { RfqGroupService } from '../rfq-group.service';
import { GroupPolicyType } from '@config/rfq';
import { GroupDocumentsDto, IGroupDocumentsDto, IGroupQNDocumentsDto, GroupQNDocumentsDto } from '@models/dtos';
import { RfqService } from '../../rfq.service';

@Component({
  selector: 'gnx-Group-qn-by-uw',
  templateUrl: './Group-qn-by-uw.component.html',
  styleUrls: ['./Group-qn-by-uw.component.scss'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class GroupQnByUwComponent {
  ...
  get QNDocuments() {
    return this.GroupQNUWForm.get('QNDocuments') as FormArray;
  }

  // get Documents Form Array 
  get Documents() {
    return this.GroupQNUWForm.get('Documents') as FormArray;
  }
  ...
  ngOnInit(): void {
    ...
    // build Group form
    this.GroupQNUWForm = this._buildForm(this.DisplayForm);
  ...
  // Reject Button 
  public RejectButton() {
    if (this.GroupQNUWForm.get('SendBackRejectDesc').value == "" || this.GroupQNUWForm.get('SendBackRejectDesc').value == null) {
    ...
    this._dialogService
      ...
      .subscribe((res) => {
        if (res) {
          this._RFQGroupService.Reject(this.GroupQNUWForm.value).subscribe((res) => {
            if (res.Success) {
  ...
  public SendBackButton() {
    ...
    this._dialogService
      ...
      .subscribe((res) => {
        if (res) {
          this._RFQGroupService.SendBack(this.GroupQNUWForm.value).subscribe((res) => {
            if (res.Success) {
              this._alertservice.raiseSuccessAlert(res.Message, "false")
              this._router.navigate([ROUTING_PATH.Basic.Dashboard])
            }
  ...
  public SubmitForm() {
    ...
    this._RFQGroupService.SubmitQuotation(this.GroupQNUWForm.value).subscribe(res => {
      if (res.Success) {
        this._alertservice.raiseSuccessAlert(res.Message, "false")
        this._router.navigate([ROUTING_PATH.Basic.Dashboard])
      }
        ...
        .subscribe((res) => {
          if (res.Success) {
            ...
            this.QNDocuments.controls[index].patchValue({
              ...
              RFQId: this.GroupQNUWForm.get('Id').value
            })
          }
          else {
            if (res.Alerts && res.Alerts?.length > 0) {
  ...
  */
  ...
  public addQNDocuments() {
    ...
    if (this.QNDocAlerts.length > 0) {
      ...
      var row: IGroupQNDocumentsDto = new GroupQNDocumentsDto()
  ...
  */
  ...
  // Build Main Form
  private _buildForm(data: IGroupQNbyUWDTO) {
