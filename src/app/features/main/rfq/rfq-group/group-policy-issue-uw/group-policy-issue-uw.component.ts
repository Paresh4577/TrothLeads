...
import { GroupDocumentsDto, IGroupDocumentsDto } from '@models/dtos';
import { IGroupPolicyIssueDto } from '@models/dtos';
import { ROUTING_PATH } from '@config/routingPath.config';
import { IPANomineeDetailDto, PANomineeDetailDto } from '@models/dtos';
import { UserTypeEnum } from 'src/app/shared/enums';
import { environment } from 'src/environments/environment';

@Component({
  selector: 'gnx-group-policy-issue-uw',
  templateUrl: './group-policy-issue-uw.component.html',
  styleUrls: ['./group-policy-issue-uw.component.scss'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class GroupPolicyIssueUwComponent {
 ...
 */
  ...
  public SubmitFormButton() {
    ...
    this._rfqGroupService.SubmitPolicyIssue(this.PolicyIssueForm.value).subscribe(res => {
      if (res.Success) {
        this._alertservice.raiseSuccessAlert(res.Message, "false")
        this._router.navigate([ROUTING_PATH.Basic.Dashboard])
      }
  ...
  */
  ...
  // Init Form
  private _initForm(data: IGroupPolicyIssueDto) {
  ...
  // Document formgroup
  private _initDocumentsForm(item: IGroupDocumentsDto): FormGroup {
