...
import { GroupDocumentsDto, IGroupDocumentsDto, IGroupPaymentDetailsDto, GroupPaymentDetailsDto } from '@models/dtos';

const ActiveMasterDataRule: IFilterRule = { Field: 'Status', Operator: 'eq', Value: 1 }
@Component({
  selector: 'gnx-group-payment-proof-sp',
  templateUrl: './group-payment-proof-sp.component.html',
  styleUrls: ['./group-payment-proof-sp.component.scss'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class GroupPaymentProofSpComponent {
 ...
 */
  ...
  public SubmitForm() {
    ...
    this._RFQGroupService.SubmitPaymentProof(this.PaymentProofForm.value).subscribe(res => {
      if (res.Success) {
        this._alertservice.raiseSuccessAlert(res.Message, "false")
        this._router.navigate([ROUTING_PATH.Basic.Dashboard])
      }
  ...
  */
  ...
  public addPaymentDetails() {
    ...
    if (this.AttachPaymentProofArrayAlerts.length > 0) {
      ...
      var row: IGroupPaymentDetailsDto = new GroupPaymentDetailsDto()
      row.RFQId = this.PaymentProofForm.get("Id").value;
  ...
  */
  ...
  private _buildPaymentDetailsForm(items: IGroupPaymentDetailsDto[] = []): FormArray {
    let formArray: FormArray = new FormArray([]);
    if (items != null) {
      if (!(items && items.length > 0)) {
        return formArray;
  ...
  private _initPaymentDetailsForm(data: IGroupPaymentDetailsDto): FormGroup {
