...
@Component({
  selector: 'gnx-group-proposal-submission-uw',
  templateUrl: './group-proposal-submission-uw.component.html',
  styleUrls: ['./group-proposal-submission-uw.component.scss'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class GroupProposalSubmissionUwComponent {
  ...
  public SubmitForm() {
    ...
    this._RFQGroupService.SubmitProposalSubmission(submitFormValue).subscribe(res => {
      if (res.Success) {
        this._alertservice.raiseSuccessAlert(res.Message, "false")
        this._router.navigate([ROUTING_PATH.Basic.Dashboard])
      }
