...
import { MiscellaneousDocumentsDto, IMiscellaneousDocumentsDto } from '@models/dtos';
import { IMiscellaneousPaymentDetailsDto, MiscellaneousPaymentDetailsDto } from '@models/dtos';
...
@Component({
  selector: 'gnx-miscellaneous-payment-proof-sp',
  templateUrl: './miscellaneous-payment-proof-sp.component.html',
  styleUrls: ['./miscellaneous-payment-proof-sp.component.scss'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class MiscellaneousPaymentProofSpComponent {
 ...
 */
  ...
  public SubmitForm() {
    ...
    this._RfqMiscellaneousService.SubmitPaymentProof(this.PaymentProofForm.value).subscribe(res => {
      if (res.Success) {
        this._alertservice.raiseSuccessAlert(res.Message, "false")
        this._router.navigate([ROUTING_PATH.Basic.Dashboard])
      }
  ...
  */
  ...
  public addPaymentDetails() {
    ...
    if (this.AttachPaymentProofAlerts.length > 0) {
      ...
      var row: IMiscellaneousPaymentDetailsDto = new MiscellaneousPaymentDetailsDto()
      row.RFQId = this.PaymentProofForm.get("Id").value;
 ...
 */
  ...
  private _buildPaymentDetailsForm(items: IMiscellaneousPaymentDetailsDto[] = []): FormArray {
    let formArray: FormArray = new FormArray([]);
    if (items != null) {
      if (!(items && items.length > 0)) {
        return formArray;
  ...
  private _initPaymentDetailsForm(data: IMiscellaneousPaymentDetailsDto): FormGroup {
