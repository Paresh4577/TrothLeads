...
import { IMiscellaneousDTO, MiscellaneousDTO } from '@models/dtos';
...
import { MiscellaneousRenewalPolicyType, RFQDocumentsDrpList } from '@config/rfq';
...
import { MiscellaneousDocumentsDto } from '@models/dtos';
import { environment } from 'src/environments/environment';
import { ROUTING_PATH } from '@config/routingPath.config';
import { RfqMiscellaneousService } from '../rfq-miscellaneous.service';
import { MiscellaneousPolicyType } from '@config/rfq';

const ActiveMasterDataRule: IFilterRule = { Field: 'Status', Operator: 'eq', Value: 1 }

@Component({
  selector: 'gnx-miscellaneous-raise',
  templateUrl: './miscellaneous-raise.component.html',
  styleUrls: ['./miscellaneous-raise.component.scss'],
  providers: [
    DatePipe,
    {
      provide: DateAdapter,
...
export class MiscellaneousRaiseComponent {
  @ViewChild('DocumentDropdown') DocumentDropdown: ElementRef;

  //Variables
  pagetitle: string = "RFQ (Requisition for Quotation) - Engineering"; // Page main header title
  ...
  SubCategoryList = [];
  EngineeringCategoryTypeList = [];
  CommodityTypeList = [];

  //FormGroup 
  RFQEngineeringForm !: FormGroup;
  RFQEngineering: IMiscellaneousDTO
  destroy$: Subject<any>;
  ...
  ngOnInit(): void {
    ...
    this.RFQEngineering = new MiscellaneousDTO();
    ...
    this.RFQEngineeringForm = this._buildForm(this.RFQEngineering);

    // in case of view mode then all form value is disabled mode
    if (this.mode == "view") {
      this.RFQEngineeringForm.disable({ emitEvent: false });
      this.isExpand = true;
    }
  ...
  get Documents() {
    return this.RFQEngineeringForm.controls["Documents"] as FormArray;
  }
  ...
  get MiscellaneousPolicyType() {
    if (this.DisplayForm?.TransactionId) {
      return MiscellaneousRenewalPolicyType;
    }
    else {
      return MiscellaneousPolicyType;
    }
  }
  ...
  get canEditableSalesPerson() {
    if (this.UserProfileObj?.UserType == UserTypeEnum.StandardUser && this.mode != 'view') {
      if (this.RFQEngineeringForm.get('SalesPersonType').value == SalesPersonTypeEnum.POSP) {
        return true;
      } else {
        return false;
      }
 ...
 */
  get canEditableBdoBdm() {
    if (this.UserProfileObj?.UserType == UserTypeEnum.StandardUser && this.mode != 'view') {
      if (this.RFQEngineeringForm.get('SalesPersonType').value == SalesPersonTypeEnum.Direct) {
        return true;
      } else {
        return false;
      }
  ...
  public AutocompleteSelectedEvent(event: MatAutocompleteSelectedEvent, SelectedFor: string): void {
    ...
    switch (SelectedFor) {
      ...
      case "TeamRef":
        this.RFQEngineeringForm.patchValue({
  ...
  public clear(name: string, id: string): void {
    this.RFQEngineeringForm.controls[name].setValue("")
    this.RFQEngineeringForm.controls[id].setValue(null)
  }
  ...
  */
  ...
  public BasicDetailsValidations() {
    ...
    if (this.RFQEngineeringForm.get('SubCategoryId').value == 0 || this.RFQEngineeringForm.get('SubCategoryId').value == null) {
    ...
    if (this.RFQEngineeringForm.get('PolicyType').hasError('required')) {
      this.BasicDetailsAlert.push({
  ...
  public ProductCategoryDetailsValidations() {
    ...
    if (this.RFQEngineeringForm.get('ProposerName').invalid) {
    ...
    // if (this.RFQEngineeringForm.get('PincodeId').invalid || this.RFQEngineeringForm.get('PincodeId').value == 0) {
    //   this.ProductCategoryDetailsAlert.push({
    ...
    if (!this.RFQEngineeringForm.get('ProposerMobileNo').value) {
      this.ProductCategoryDetailsAlert.push({
      ...
      if (
        !this.phoneNum.test(this.RFQEngineeringForm.get('ProposerMobileNo').value)
      ) {
        this.ProductCategoryDetailsAlert.push({
    ...
    if (this.RFQEngineeringForm.get('ProposerEmail').value != "" && this.RFQEngineeringForm.get('ProposerEmail').value != null) {
      if (!this.emailValidationReg.test(this.RFQEngineeringForm.get('ProposerEmail').value)) {
        this.ProductCategoryDetailsAlert.push({
    ...
    if (!this.RFQEngineeringForm.get('SumInsured').value) {
      this.ProductCategoryDetailsAlert.push({
  ...
  public PreviousPolicyDetailsValidations() {
    ...
    if (this.RFQEngineeringForm.get('PolicyType').value == 'Rollover' || this.RFQEngineeringForm.get('PolicyType').value == 'Renewal-Change Company' || this.RFQEngineeringForm.get('PolicyType').value == 'Renewal-Same Company') {
      if (this.RFQEngineeringForm.get('PreviousPolicyStartDate').value && this.RFQEngineeringForm.get('PreviousPolicyEndDate').value && this.RFQEngineeringForm.get('PreviousPolicyEndDate').value < this.RFQEngineeringForm.get('PreviousPolicyStartDate').value) {
  ...
  public TeamDetailsValidations() {
    ...
    if (this.RFQEngineeringForm.get('BranchId').invalid || this.RFQEngineeringForm.get('BranchId').value == 0) {
    ...
    if (this.RFQEngineeringForm.get('SalesPersonType').invalid || this.RFQEngineeringForm.get('SalesPersonType').value == "") {
      this.TeamDetailsAlerts.push({
    ...
    if (this.RFQEngineeringForm.get('SalesPersonName').invalid || this.RFQEngineeringForm.get('SalesPersonName').value == "") {
      this.TeamDetailsAlerts.push({
    ...
    if (this.RFQEngineeringForm.get('SalesPersonType').value == 'Team Reference') {
      if (this.RFQEngineeringForm.get('TeamReferenceName').invalid || this.RFQEngineeringForm.get('TeamReferenceName').value == "") {
        this.TeamDetailsAlerts.push({
    ...
    if (!this.RFQEngineeringForm.get('BDMName').value) {
      this.TeamDetailsAlerts.push({
    ...
    if (!this.RFQEngineeringForm.get('BDOName').value) {
      this.TeamDetailsAlerts.push({
  ...
  */
  public addDocuments(selectedDocument?: string) {
    const row: MiscellaneousDocumentsDto = new MiscellaneousDocumentsDto();
...
*/
  ...
  public SubmitRfqEngineering() {
    ...
    switch (this.mode) {
      case "create": case "RenewalRFQ":
        this._RfqMiscellaneousService.CreateProposal(this.RFQEngineeringForm.value).subscribe((res) => {
          if (res.Success) {
      ...
      case "edit":
        this._RfqMiscellaneousService.UpdateProposal(this.RFQEngineeringForm.value).subscribe((res) => {
          if (res.Success) {
            this._alertservice.raiseSuccessAlert(res.Message, "false")
            this._router.navigate([ROUTING_PATH.Basic.Dashboard])
          }
  ...
  // Build RFQ Engineering Main Form
  private _buildForm(data: IMiscellaneousDTO) {
  ...
  //RFQ-Engineering document Formarray
  private _buildDocumentsForm(items: MiscellaneousDocumentsDto[] = []): FormArray {
  ...
  private _initDocumentsForm(item: MiscellaneousDocumentsDto): FormGroup {
    ...
    if (item != null) {
      if (!item) {
        item = new MiscellaneousDocumentsDto();
      }
      if (item) {
        dF.patchValue(item);
      }
  ...
  private _onFormChange() {
    ...
    this.RFQEngineeringForm.get('SubCategoryId').valueChanges.subscribe(val => {
    ...
    this.RFQEngineeringForm.get('PolicyType').valueChanges.subscribe(val => {
      if (val == "New") {
      this.RFQEngineeringForm.patchValue({
        PrevPolicyInsurComp: "",
        PrevPolicySumInsured: 0,
    ...
    // change sales person
    this.RFQEngineeringForm.get('SalesPersonName').valueChanges.subscribe((val) => {
    ...
    this.RFQEngineeringForm.get('TeamReferenceName').valueChanges.subscribe(
      (val) => {
    ...
    this.RFQEngineeringForm.get('TeamReferenceId').valueChanges.subscribe(
      (val) => {
        if (!val && this.UserProfileObj.UserType == UserTypeEnum.StandardUser && this.RFQEngineeringForm.get('SalesPersonType').value == SalesPersonTypeEnum.TeamReference) {
          this.RFQEngineeringForm.patchValue({
            BDMId: null,
            BDMName: null,
            BDOId: null,
    ...
    this.RFQEngineeringForm.get('SalesPersonId').valueChanges.subscribe(
      (val) => {
        if (!val && this.UserProfileObj.UserType == UserTypeEnum.StandardUser && this.RFQEngineeringForm.get('SalesPersonType').value == SalesPersonTypeEnum.POSP) {
          this.RFQEngineeringForm.patchValue({
            BDMId: null,
            BDMName: null,
            BDOId: null,
    ...
    this.RFQEngineeringForm.get('Pincode').valueChanges.subscribe((val) => {
     ...
     */
    this.RFQEngineeringForm.get('BranchId').valueChanges.subscribe((val) => {
      this._TeamDetailsForStandardUser()
    })


    this.RFQEngineeringForm.get('SalesPersonType').valueChanges.subscribe((val) => {
      this._TeamDetailsForStandardUser()
    })
    ...
    */
    this.RFQEngineeringForm.get('BDOName').valueChanges.subscribe((val) => {
     ...
     */
    this.RFQEngineeringForm.get('BDMName').valueChanges.subscribe((val) => {
  ...
  private _fillMasterList() {
    ...
    let SubCategoryRule: IFilterRule[] = [ActiveMasterDataRule,
      { Field: "Category.Code", Operator: "eq", Value: CategoryCodeEnum.Miscellaneous }
    ]
    ...
    let InsuranceCompanyAdditionalFilters: IAdditionalFilterObject[] = [{ key: "CatagoryCode", filterValues: [CategoryCodeEnum.Miscellaneous] }]
  ...
  private _dateFormat() {
    this.RFQEngineeringForm.patchValue({
      ProjectStartDate: this._datePipe.transform(this.RFQEngineeringForm.get('ProjectStartDate')?.value, 'yyyy-MM-dd'),
      PreviousPolicyStartDate: this._datePipe.transform(this.RFQEngineeringForm.get('PreviousPolicyStartDate')?.value, 'yyyy-MM-dd'),
      PreviousPolicyEndDate: this._datePipe.transform(this.RFQEngineeringForm.get('PreviousPolicyEndDate')?.value, 'yyyy-MM-dd'),
    }, { emitEvent: false })
  }
    ...
    this.authService.userProfile$.subscribe((user: IMyProfile) => {
      if (user) {
        ...
        if (user.UserType == UserTypeEnum.Agent) {
          ...
          if (this.RFQEngineeringForm.value?.BranchId) {

            let LoginUserBranch = this.Branches.find(b => b.Id == this.RFQEngineeringForm.value?.BranchId)
            if (LoginUserBranch) {
              this.RFQEngineeringForm.patchValue({
  ...
  private _TeamDetailsForStandardUser() {
    if (this.UserProfileObj.UserType == UserTypeEnum.StandardUser) {
      ...
      if (this.RFQEngineeringForm.get('SalesPersonType').value == SalesPersonTypeEnum.Direct) {
        ...
        let LoginUserBranch = this.Branches.find(b => b.Id == this.RFQEngineeringForm.get('BranchId').value)
      ...
      } else if (this.RFQEngineeringForm.get('SalesPersonType').value == SalesPersonTypeEnum.POSP) {
         ...
         * Other Field is null
         */
      } else if (this.RFQEngineeringForm.get('SalesPersonType').value == SalesPersonTypeEnum.TeamReference) {
...
*/
  private _salesPersonListAPIfilter(): QuerySpecs {
     ...
     */
    if (this.RFQEngineeringForm.get('SalesPersonType').value == "Direct") {
      specs.FilterConditions.Rules = [ActiveMasterDataRule,
        { Field: 'Branch.Id', Operator: 'eq', Value: this.RFQEngineeringForm.get('BranchId').value, }
      ]
    }

    if (this.RFQEngineeringForm.get('SalesPersonType').value == "POSP") {
      specs.FilterConditions.Rules = [
    ...
    else if (this.RFQEngineeringForm.get('SalesPersonType').value == "POSP") {
  ...
  */
  private _teamReferenceListAPIfilter(): QuerySpecs {
    ...
    else if (this.RFQEngineeringForm.get('SalesPersonType').value == "Team Reference") {
  ...
  */
  private _bdoListAPIfilter(): QuerySpecs {
    ...
    if (this.RFQEngineeringForm.get('SalesPersonType').value == "Direct") {
      ...
      if (this.RFQEngineeringForm.get('BranchId').value) {
        specs.AdditionalFilters.push({ key: 'Branch', filterValues: [this.RFQEngineeringForm.get('BranchId').value?.toString()] })
      }
    }
