...
//Config - temp - this is to come from api or from config.ts file
const config: any = {
  screens: {
    sm: '600px',
    md: '960px',
...
const config$: Observable<any> = of(config);
//
...
export class GnxMediaWatcherService {
  ...
  constructor(
    ...
    config$.pipe(
      ...
      return  this._breakpointObserver.observe(Object.values(screens)).pipe(
          map((state) => {
